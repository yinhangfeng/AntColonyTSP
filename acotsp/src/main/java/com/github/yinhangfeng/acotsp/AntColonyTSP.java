...
/**
 * 蚁群算法求解 TSP
 * 参考 https://blog.csdn.net/peiwang245/article/details/78072130
 */
public class AntColonyTSP {
    ...
    private IDistance distanceFunc = EuclideanDistance.INSTANCE;
     ...
     * 距离计算函数
     */
    public AntColonyTSP distanceFunc(IDistance distanceFunc) {
    ...
    public Response run(List<Point> points) {
        ...
        if (pointCount > maxExhaustiveCount) {
            return runACO(points, maxIterations <= 0 ? Math.max(pointCount, 16) : maxIterations);
        }
...
//        assert Math.abs(l2 - newBestLength) < 0.0000001;
        ...
        response.originalPoints = points;
        response.tour = tour;
    ...
    private void initDistance(List<Point> points, boolean initEtaPowerBeta) {
        ...
        for (int i = 0; i < pointCount; ++i) {
            ...
            for (int j = i + 1; j < pointCount; ++j) {
                weight = distanceFunc.getDistance(points.get(i), points.get(j));
    ...
    private void visitNext(Ant ant, int nextIndex) {
        ...
        // TODO totalWeight 有可能为 0 Infinity ?
...
//        System.out.println("updateTauPowerAlphaMultiplyEtaPowerBeta:\n" + Arrays.deepToString(tauPowerAlphaMultiplyEtaPowerBeta));
    ...
    public void recycle() {
        ...
        synchronized (sCache) {
            ...
            distanceFunc = EuclideanDistance.INSTANCE;
            int i = sCache.size() - 1;
            for (; i >= 0; --i) {
                AntColonyTSP inst = sCache.get(i);
